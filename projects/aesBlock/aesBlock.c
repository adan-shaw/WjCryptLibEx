...
static void ReadHexData (char const *HexString, uint8_t * Data, uint32_t * pDataSize)
{
	...
	{
		if ((HexString[i] >= '0' && HexString[i] <= '9') || (HexString[i] >= 'A' && HexString[i] <= 'F') || (HexString[i] >= 'a' && HexString[i] <= 'f'))
		{
...
int main (int ArgC, char **ArgV)
{
	...
	for (i = 1; i < (uint32_t) ArgC; i++)
	{
		...
		else
		{
			...
			{
				bufferPtr = key;
				bufferSizePtr = &keySize;
			}
			else if (1 == paramIndex)
			{
				bufferPtr = block;
				bufferSizePtr = &blockSize;
			}
			else
			{
	...
	{
	case 128 / 8:
		AesInitialise (&aesContext, key, AES_KEY_SIZE_128);
		break;
	case 192 / 8:
		AesInitialise (&aesContext, key, AES_KEY_SIZE_192);
		break;
	case 256 / 8:
		AesInitialise (&aesContext, key, AES_KEY_SIZE_256);
		break;
	default:
